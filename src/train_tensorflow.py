...
def train_with_hyperparameter(epochs, batch_size, learning_rate, lstm_layers, dropout):
    ...
    X_train, X_test, y_train_emotion, y_test_emotion, y_train_toxicity, y_test_toxicity = train_test_split(X, y_emotion_combined,y_toxicity_combined, test_size=0.2, random_state=42, shuffle=False) 
    # initilize optimizer
    ...
    try:
        ...
        print("Saving the results")
        # Format of file 'dat'
        save_to_json(history.history, f'{datetime.now().strftime("%Y%m%d%H%M%S")}_mtm_with_bert_{epochs}_{batch_size}_adam_{layer_1}_{layer_2}_with_regularization_history.json')
